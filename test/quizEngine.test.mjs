...
test('quiz engine no longer exposes the fixed 10-question quiz mode', () => {
  assert.equal('createQuiz' in quizEngine, false);
});

test('streak quiz can ask blade, ratchet, and bit questions', () => {
  const types = new Set(collectStreakAnswerTypes(BEYBLADE_X_QUIZ_ITEMS.length, 'mixed-parts-challenge'));

  assert.deepEqual(types, new Set(['blade', 'ratchet', 'bit']));
});

test('streak quiz includes every part category before reshuffling', () => {
  for (const seed of ['balanced-1', 'balanced-2', 'balanced-3']) {
    const types = new Set(collectStreakAnswerTypes(BEYBLADE_X_QUIZ_ITEMS.length, seed));

    assert.deepEqual(types, new Set(['blade', 'ratchet', 'bit']));
  }
});

test('createStreakQuiz builds a 4-choice set with one correct same-part answer', () => {
  const quiz = createStreakQuiz(BEYBLADE_X_QUIZ_ITEMS, streakOptions);
...
test('seeded streak quiz order is repeatable', () => {
  const first = collectStreakAnswerIds(12, 'repeatable-order');
  const second = collectStreakAnswerIds(12, 'repeatable-order');
...
test('timeOutCurrentQuestion marks unanswered current question as wrong and ends the challenge', () => {
  const quiz = createStreakQuiz(BEYBLADE_X_QUIZ_ITEMS, streakOptions);
...
test('streak quiz starts without a fixed final question count', () => {
  ...
  assert.equal(quiz.questionCount, undefined);
  assert.equal(quiz.passingScore, undefined);
  assert.equal(question.options.length, 4);
  assert.deepEqual(
    new Set(question.options.map((option) => option.type)),
    new Set([question.answer.type]),
...
test('createStreakQuiz rejects invalid quiz settings before building state', () => {
  assert.throws(
    () => createStreakQuiz(BEYBLADE_X_QUIZ_ITEMS, { ...streakOptions, optionsPerQuestion: 1 }),
    /optionsPerQuestion/,
  );
  assert.throws(
    () => createStreakQuiz(BEYBLADE_X_QUIZ_ITEMS, { ...streakOptions, optionsPerQuestion: 2.5 }),
    /optionsPerQuestion/,
  );
});

test('getQuizResult reports only the current streak result', () => {
  let quiz = createStreakQuiz(BEYBLADE_X_QUIZ_ITEMS, streakOptions);
  const firstQuestion = getCurrentQuestion(quiz);
  quiz = answerCurrentQuestion(quiz, firstQuestion.answer.id);
  quiz = advanceQuestion(quiz);
  ...
  assert.deepEqual(getQuizResult(quiz), {
    answeredCount: 2,
    score: 1,
    streak: 1,
  });
...
function collectStreakAnswerIds(count, seed) {
  let quiz = createStreakQuiz(BEYBLADE_X_QUIZ_ITEMS, {
    ...streakOptions,
    seed,
  });
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    quiz = answerCurrentQuestion(quiz, question.answer.id);
...
function collectStreakAnswerTypes(count, seed) {
  return collectStreakAnswerIds(count, seed).map((id) => {
    return BEYBLADE_X_QUIZ_ITEMS.find((item) => item.id === id).type;
  });
}
